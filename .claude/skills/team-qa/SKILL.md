...
**Decision Points:** At each phase transition, use `AskUserQuestion` or `request_user_input` or `clarify` to present
...
Present the qa-lead's full strategy to the user, then use `AskUserQuestion` or `request_user_input` or `clarify`:
...
Use `AskUserQuestion` or `request_user_input` or `clarify` per story group (batched 3-4 at a time):
...
Batch stories into groups of 3-4 and use `AskUserQuestion` or `request_user_input` or `clarify` for each:
...
After each FAIL result: use `AskUserQuestion` or `request_user_input` or `clarify` to collect the failure description, then spawn `qa-tester` via Task to write a formal bug report in `production/qa/bugs/`.
