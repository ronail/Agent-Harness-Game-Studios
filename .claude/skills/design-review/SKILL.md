...
Use `AskUserQuestion` or `request_user_input` or `clarify` for ALL closing interactions. Never plain text.
...
After all revisions are complete, show a summary table (blocker → fix applied) and use `AskUserQuestion` or `request_user_input` or `clarify` for a **post-revision closing widget**:
