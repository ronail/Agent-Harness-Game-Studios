...
user-invocable: true
allowed-tools: Read, Glob, Grep, Write, WebSearch, Task, AskUserQuestion, request_user_input, clarify
model: sonnet
---
...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use `AskUserQuestion` or `request_user_input` or `clarify`** at key decision points throughout brainstorming:
...
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Use `AskUserQuestion` or `request_user_input` or `clarify` for this — constrained choice.)*
...
Present all three. Then use `AskUserQuestion` or `request_user_input` or `clarify` to capture the selection.
...
**Pillar confirmation**: After presenting the full pillar set, use `AskUserQuestion` or `request_user_input` or `clarify`:
...
If the user selects B, C, or D, make the revision, then use `AskUserQuestion` or `request_user_input` or `clarify` again:
...
- **Target platform**: Use `AskUserQuestion` or `request_user_input` or `clarify` — "What platforms are you targeting for this game?"
...
- **Engine experience**: Use `AskUserQuestion` or `request_user_input` or `clarify` — "Do you already have an engine you work in?"
...
5. Use `AskUserQuestion` or `request_user_input` or `clarify` for write approval:
...
If [B]: ask which section to revise using `AskUserQuestion` or `request_user_input` or `clarify` with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else — I'll describe`

After revising, show the updated section as a diff or clear before/after, then use `AskUserQuestion` or `request_user_input` or `clarify` — "Ready to write the updated concept document?"
