...
user-invocable: true
allowed-tools: Read, Glob, Grep, Bash, Write, Task, AskUserQuestion, request_user_input, clarify
model: opus
---
...
After the verdict is presented and any stage.txt update is complete, close with a structured next-step prompt using `AskUserQuestion` or `request_user_input` or `clarify`.
