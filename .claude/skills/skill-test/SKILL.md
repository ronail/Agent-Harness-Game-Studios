...
- `category [name]` → run category-specific rubric from `AHGS Skill Testing Framework/quality-rubric.md`
...
Look up the spec path from `AHGS Skill Testing Framework/catalog.yaml` — use the
...
Date: [date]
Spec: AHGS Skill Testing Framework/skills/[category]/[name].md
...
"May I write these results to `AHGS Skill Testing Framework/results/skill-test-spec-[name]-[date].md`
and update `AHGS Skill Testing Framework/catalog.yaml`?"

If yes:
- Write results file to `AHGS Skill Testing Framework/results/`
- Update the skill's entry in `AHGS Skill Testing Framework/catalog.yaml`:
  - `last_spec: [date]`
...
Find skill at `.claude/skills/[name]/SKILL.md`.
Look up `category:` field in `AHGS Skill Testing Framework/catalog.yaml`.
...
Read `AHGS Skill Testing Framework/quality-rubric.md`.
...
"May I update `AHGS Skill Testing Framework/catalog.yaml` to record this category check
...
Read `AHGS Skill Testing Framework/catalog.yaml`. If missing, note that catalog doesn't exist
yet (first-run state).
...
Also read the `agents:` section from `AHGS Skill Testing Framework/catalog.yaml` to get the
complete list of agents.

### Step 3 — Build Skill Coverage Table

For each skill:
- Check if a spec file exists (use the `spec:` path from catalog, or glob `AHGS Skill Testing Framework/skills/*/[name].md`)
...
- Check if a spec file exists (use the `spec:` path from catalog, or glob `AHGS Skill Testing Framework/agents/*/[name].md`)
...
- After `spec [name]` PASS: "Update `AHGS Skill Testing Framework/catalog.yaml` to record this
...
- After `audit`: "Start with the critical-priority gaps. Use the spec template
  at `AHGS Skill Testing Framework/templates/skill-test-spec.md` to create new specs."
