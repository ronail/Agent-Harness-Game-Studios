...
- If it exists: read it, find the first entity or screen with status "Needed" but no spec file yet, and use `AskUserQuestion`, `request_user_input` or `clarify`:
...
Present the full proposed inventory to the user in conversation. Then use `AskUserQuestion`, `request_user_input` or `clarify`:
...
- **system**: Read `design/gdd/[target-name].md`. Extract the **Visual/Audio Requirements** section. If it doesn't exist or reads `[To be designed]`:
  ...
  Use `AskUserQuestion`, `request_user_input` or `clarify`: `[A] Describe needs manually` / `[B] Stop — complete the GDD first`
...
- **character** or **entity**: Read `design/narrative/characters/[target-name].md` or search `design/narrative/` and `design/assets/entity-inventory.md` for a matching entry. Extract visual description, role, and any specified distinguishing features.
  - **If no source doc exists**: do not fail. Instead, use `AskUserQuestion`, `request_user_input` or `clarify`:
...
Present the full identified list to the user. Use `AskUserQuestion`, `request_user_input` or `clarify`:
...
After presenting all specs, use `AskUserQuestion`, `request_user_input` or `clarify`:
