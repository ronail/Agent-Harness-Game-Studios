...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use `AskUserQuestion` or `request_user_input` or `clarify`:
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use `AskUserQuestion` or `request_user_input` or `clarify` to capture the decision.
...
**Framing questions (ask BEFORE drafting)**: Use `AskUserQuestion` or `request_user_input` or `clarify` with a multi-tab widget:
...
**Framing question (ask BEFORE drafting)**: Use `AskUserQuestion` or `request_user_input` or `clarify`:
...
- Surface any disagreements between agents to the user via `AskUserQuestion` or `request_user_input` or clarify
...
- Present the specialists' proposals to the user for review via `AskUserQuestion` or `request_user_input` or clarify
...
- The main session presents the agent's output to the user via `AskUserQuestion` or `request_user_input` or clarify
- The user decides; the main session writes to file
