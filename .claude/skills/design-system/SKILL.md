...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use `AskUserQuestion` or `clarify`:
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use `AskUserQuestion` or `clarify` to capture the decision.
...
**Framing questions (ask BEFORE drafting)**: Use `AskUserQuestion` or `clarify` with a multi-tab widget:
...
**Framing question (ask BEFORE drafting)**: Use `AskUserQuestion` or `clarify`:
