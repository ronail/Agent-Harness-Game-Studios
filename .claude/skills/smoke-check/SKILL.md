...
user-invocable: true
allowed-tools: Read, Glob, Grep, Bash, Write, AskUserQuestion, request_user_input, clarify
model: sonnet
---
...
Use `AskUserQuestion` or `request_user_input` or `clarify` to batch-verify. Keep to at most 3 calls.
...
- Use `AskUserQuestion` or `request_user_input` or `clarify` for all manual smoke check verification.
