...
user-invocable: true
allowed-tools: Read, Glob, Grep, Bash, Write, AskUserQuestion, clarify
model: sonnet
---
...
Use `AskUserQuestion` or `clarify` to batch-verify. Keep to at most 3 calls.
...
- Use `AskUserQuestion` or `clarify` for all manual smoke check verification.
