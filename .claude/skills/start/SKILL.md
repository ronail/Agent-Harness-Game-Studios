...
This is the first thing the user sees. Use `AskUserQuestion` or `request_user_input` or `clarify` with these exact options so the user can click rather than type:
...
2. Acknowledge the concept, then use `AskUserQuestion` or `request_user_input` or `clarify` to offer two paths:
...
- **If Hermes is installed**: Discover available profiles via `Bash`
  ...
  `AskUserQuestion` or `request_user_input` or `clarify` to ask:
...
The standard tier is the project default. Use `AskUserQuestion` or `request_user_input` or `clarify` to
ask:
...
After presenting the recommended path, use `AskUserQuestion` or `request_user_input` or `clarify` to ask the user which step they'd like to take first. Never auto-run the next skill.
