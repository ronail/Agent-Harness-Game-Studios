...
**If no argument is provided**, do not fail — ask instead. Use `AskUserQuestion` or `clarify`:
...
2. **Questions**: Ask what is needed to draft this section. Use `AskUserQuestion` or `clarify`
...
3. **Options**: Where design choices exist, present 2-4 approaches with pros/cons.
   Explain reasoning in conversation, then use `AskUserQuestion` or `clarify` to capture the decision.
4. **Decision**: User picks an approach or provides custom direction.
5. **Draft**: Write the section content in conversation for review. Flag provisional
   assumptions explicitly.
6. **Approval**: Use `AskUserQuestion` or `clarify`:
...
7. **Write**: Use `AskUserQuestion` or `clarify`: "May I write the [section name] section to `[filepath]`?"
...
- Use `AskUserQuestion` or `clarify` to capture the choice:
...
- Use `AskUserQuestion` or `clarify`: "Want an ASCII wireframe as part of this spec?"
...
Use `AskUserQuestion` or `clarify` to confirm:
...
Use `AskUserQuestion` or `clarify` to step through items in groups of 3-4, not all at once.
...
Work through patterns in groups. Use `AskUserQuestion` or `clarify`:
