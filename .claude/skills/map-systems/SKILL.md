...
- Use `AskUserQuestion`, `request_user_input` or `clarify` to ask:
...
Then use `AskUserQuestion`, `request_user_input` or `clarify` to capture feedback:
...
Use `AskUserQuestion`, `request_user_input` or `clarify` to ask: "Does this dependency ordering look right? Any
...
Use `AskUserQuestion`, `request_user_input` or `clarify` to ask: "Do these priority assignments match your vision?
...
Use `AskUserQuestion`, `request_user_input` or `clarify` for: "Start designing [system-name] now, pick a different
system, or stop here?"
...
After `/design-system` completes, use `AskUserQuestion`, `request_user_input` or `clarify`:
...
After the systems index is created (or after designing some systems), present next actions using `AskUserQuestion`, `request_user_input` or `clarify`:
