...
Then use `AskUserQuestion` or `request_user_input` or `clarify` to collect the **category**:
...
Then use `AskUserQuestion` or `request_user_input` or `clarify` to collect the **estimated fix effort**:
