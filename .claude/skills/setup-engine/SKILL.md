...
user-invocable: true
allowed-tools: Read, Glob, Grep, Write, Edit, WebSearch, WebFetch, Task, AskUserQuestion, request_user_input, clarify
model: sonnet
---
...
**Question 1 — Prior experience** (ask this first, always, via `AskUserQuestion` or `request_user_input` or clarify):
...
**Question 2 — Target platform** (ask this second, always, via `AskUserQuestion` or `request_user_input` or clarify — platform eliminates or heavily weights engines before any other factor):
...
5. Use `AskUserQuestion` or `request_user_input` or `clarify` to confirm: "Does this recommendation feel right, or would you like to explore a different engine?"
...
Use `AskUserQuestion` or `request_user_input` or `clarify` with concept-specific deep-dive topics. Always generate these options from the user's actual concept — do not use generic options. Always include at minimum:
...
### Remaining Sections
- **Performance Budgets**: Use `AskUserQuestion` or `request_user_input` or `clarify`:
