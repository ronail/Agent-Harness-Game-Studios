...
**Decision Points:** At each step transition, use `AskUserQuestion` or `request_user_input` or `clarify` to present
...
**Gate**: Use `AskUserQuestion` or `request_user_input` or `clarify` to present all three Step 1 outputs (narrative brief, lore foundation, visual direction targets) and confirm before proceeding to Step 2.
...
Use `AskUserQuestion` or `request_user_input` or `clarify` with options:
...
**Gate**: Use `AskUserQuestion` or `request_user_input` or `clarify` to present Step 2 layout (including any unresolved adjacent area dependencies) and confirm before proceeding to Step 3.
...
**Gate**: Use `AskUserQuestion` or `request_user_input` or `clarify` to present Step 3 outputs and confirm before proceeding to Step 4.
...
**Gate**: Use `AskUserQuestion` or `request_user_input` or `clarify` to present both Step 4 results. If the accessibility-specialist returned any BLOCKING concerns, highlight them prominently and offer:
...
3. **Offer options** via AskUserQuestion or request_user_input or clarify with choices:
