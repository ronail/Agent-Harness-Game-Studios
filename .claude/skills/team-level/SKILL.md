...
**Gate**: Use `AskUserQuestion`, `request_user_input` or `clarify` to present all three Step 1 outputs (narrative brief, lore foundation, visual direction targets) and confirm before proceeding to Step 2.
...
Use `AskUserQuestion`, `request_user_input` or `clarify` with options:
...
**Gate**: Use `AskUserQuestion`, `request_user_input` or `clarify` to present Step 2 layout (including any unresolved adjacent area dependencies) and confirm before proceeding to Step 3.
...
**Gate**: Use `AskUserQuestion`, `request_user_input` or `clarify` to present Step 3 outputs and confirm before proceeding to Step 4.
...
**Gate**: Use `AskUserQuestion`, `request_user_input` or `clarify` to present both Step 4 results. If the accessibility-specialist returned any BLOCKING concerns, highlight them prominently and offer:
