...
If the project appears fresh (no artifacts at all), use `AskUserQuestion` or `clarify`:
...
and offer to handle it immediately using `AskUserQuestion` or `clarify`. Choose the first
branch that applies:
