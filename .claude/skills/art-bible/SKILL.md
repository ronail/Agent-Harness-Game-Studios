...
Use `AskUserQuestion` or `clarify` with two tabs:
...
Present the art-director's draft to the user. Use `AskUserQuestion` or `clarify`:
...
Collect both. If they conflict (e.g., art-director wants elaborate diegetic UI but ux-designer flags it would reduce combat readability), surface the conflict explicitly with both positions. Do NOT silently resolve — use `AskUserQuestion` or `clarify` to let the user decide.
...
Use `AskUserQuestion` or `clarify` for next steps. Only include options that are genuinely next based on the state check above:
