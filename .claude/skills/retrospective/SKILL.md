...
If a matching file is found, use `AskUserQuestion` or `request_user_input` or `clarify`:
...
Then use `AskUserQuestion` or `request_user_input` or `clarify` to present two options:
